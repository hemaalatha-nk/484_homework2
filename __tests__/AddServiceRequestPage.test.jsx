...
describe("Testing Create and List Request Functionality -> 312 App", () => {
    test("(5 pts) Create a feedback request and see if appears on list request page", async () => {
        ...
        expect(
            document
                .getElementById("main-table-body")
                .innerHTML.includes(iname) &&
                document
                    .getElementById("main-table-body")
                    .innerHTML.includes(isdesc) &&
                document
                    .getElementById("main-table-body")
                    .innerHTML.includes(ilongdesc) &&
                document
                    .getElementById("main-table-body")
                    .innerHTML.includes(iemailId),
        ).toBe(true);
        ...
        expect(
            tableDOM.rows[tableLength - 1].cells[0].innerHTML.includes(
                "<button",
            ),
        ).toBe(true);
        expect(
            tableDOM.rows[tableLength - 1].cells[5].innerHTML.includes(
                "<button",
            ),
        ).toBe(true);
    });
    ...
    test("(5 pts) Create a feedback request and see if appears on list request page and click reset button", async () => {
        ...
        expect(
            document.getElementById("name").value.includes(iname) &&
                document
                    .getElementById("sdescription")
                    .value.includes(isdesc) &&
                document
                    .getElementById("ldescription")
                    .value.includes(ilongdesc) &&
